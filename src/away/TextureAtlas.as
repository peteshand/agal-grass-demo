...
package away
{
    ...
    public class TextureAtlas
    {
        ...
        private var mAtlasTexture:Texture2DBase;
        private var mTextureRegions:Dictionary;
        private var mTextureFrames:Dictionary;
        
        /** helper objects */
        public var animationNames:Vector.<String> = new <String>[];
        public var frameNames:Vector.<String> = new <String>[];
        
        /** Create a texture atlas from a texture by parsing the regions from an XML file. */
        public function TextureAtlas(texture:Texture2DBase, atlasXml:XML=null)
        {
        ...
        protected function parseAtlasXml(atlasXml:XML):void
        {
            ...
            for each (var subTexture:XML in atlasXml.SubTexture)
            {
				...
				var prefix:String = pattern.exec(name)[0];
				
				var alreadyAdded:Boolean = false;
				for (var i:int = 0; i < animationNames.length; i++) 
				{
					if (animationNames[i] == prefix) {
						alreadyAdded = true;
						continue;
					}
				}
				if (!alreadyAdded) animationNames.push(prefix);
			...
			trace("animationNames = " + animationNames);
        }
        ...
        /*public function getTexture(name:String):Texture2DBase
        {
            ...
            else return Texture2DBase.fromTexture(mAtlasTexture, region, mTextureFrames[name]);
        }*/
        
        /** Returns all textures that start with a certain string, sorted alphabetically
         *  (especially useful for "MovieClip"). */
        /*public function getTextures(prefix:String="", result:Vector.<Texture2DBase>=null):Vector.<Texture2DBase>
        {
            if (result == null) result = new <Texture2DBase>[];
		...
		public function updateGeoByIndex(geo:PlaneGeometry, index:int):PlaneGeometry 
		{
			PlaneGeoUVUpdater.update(geo, getRegion(frameNames[index]), mAtlasTexture.width, mAtlasTexture.height);
			return geo;
		}
        ...
        public function get texture():Texture2DBase { return mAtlasTexture; }
    }
}